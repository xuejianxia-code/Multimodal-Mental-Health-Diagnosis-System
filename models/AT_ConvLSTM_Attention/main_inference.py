...
def main(dataloaders, audio_net, text_net, fusion_net, evaluator, base_logger, writer, config,config_out_dir, args, model_type, ckpt_path):
    ...
    model_parameters = [*audio_net.parameters()] + [*text_net.parameters()] + [*evaluator.parameters()]
        ...
        for mode in ['test']:
            ...
            for data in tqdm(dataloaders[mode]):
                ...
                def model_processing(input):
                    ...
                    # all_features = torch.stack([audio_features, text_features], dim=1).unsqueeze(dim=1)
...
if __name__ == '__main__':
    ...
    audio_net, text_net, fusion_net, evaluator = get_models(config['MODEL'], args, model_type, ckpt_path)

    main(dataloaders, audio_net, text_net, fusion_net, evaluator, base_logger, writer, config['MODEL'],config['OUTPUT_DIR'] ,args, model_type, ckpt_path)

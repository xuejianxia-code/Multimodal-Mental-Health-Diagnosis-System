...
#     #     return session
class DepressionDataset(Dataset):
    '''create a training, development, or test dataset
       and load the participant features if it's called
    '''

    def __init__(self, root_dir, mode, use_mel_spectrogram=True, visual_with_gaze=False, transform=None):
        super(DepressionDataset, self).__init__()
        self.visual_with_gaze = visual_with_gaze
        ...
        # Load ground truth based on the mode
        self.IDs = self.load_numpy_file('ID_gt.npy')
        self.gender_gt = self.load_numpy_file('gender_gt.npy')
        self.phq_binary_gt = self.load_numpy_file('phq_binary_gt.npy')
        self.phq_score_gt = self.load_numpy_file('phq_score_gt.npy')
        self.phq_subscores_gt = self.load_numpy_file('phq_subscores_gt.npy') if mode != 'test' else None
    ...
    def __getitem__(self, idx):
        ...
        # Get audio feature
        audio_path = self.root_dir / 'audio' / ('mel-spectrogram' if self.use_mel_spectrogram else 'spectrogram')
        audio_file = np.sort(os.listdir(audio_path))[idx]
        audio = np.load(audio_path / audio_file)  # shape: frequency_bins x num_sample (80 x 1800)

        # Get text feature
        text_path = self.root_dir / 'text'
        text_file = np.sort(os.listdir(text_path))[idx]
        text = np.load(text_path / text_file)

        # Prepare session dictionary
        session = {
            'ID': self.IDs[idx],
            'gender_gt': self.encode_gender(self.gender_gt[idx]),
            'phq_binary_gt': self.phq_binary_gt[idx],
            'phq_score_gt': self.phq_score_gt[idx],
            'audio': audio,
            'text': text
        ...
        if self.phq_subscores_gt is not None:
            session['phq_subscores_gt'] = self.phq_subscores_gt[idx]
    ...
    def encode_gender(self, gender):
        gender = gender.strip().lower()  # 去除多余的空格并转换为小写
        if gender == 'female':
            return 0
        elif gender == 'male':
            return 1

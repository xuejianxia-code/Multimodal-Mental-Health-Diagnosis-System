...
# import torch_optimizer as optim.SAM
...
def get_dataloaders(data_config,single_sample=False):

    if single_sample:
        # 修改数据加载器，设置只加载一个数据
        data_config['batch_size'] = 1  # 设置 batch_size 为 1
        data_config['shuffle'] = False  # 取消 shuffle
    dataloaders = {}
...
#     # base_optimizer = getattr(torch.optim, optimizer_config['NAME'].capitalize())
#     # # 确保学习率是浮点数
#     # lr = float(optimizer_config['LR'])  # 确保将学习率从字符串转换为浮点数
...
#         optimizer = SAM(model_parameters, base_optimizer, lr=lr, rho=2, adaptive=True, betas=(0.9, 0.999))
#
...
def get_optimizer_scheduler(model_parameters, optimizer_config, scheduler_config):
    ...
    lr = float(optimizer_config['LR'])  # Convert learning rate from string to float
    weight_decay = float(optimizer_config['WEIGHT_DECAY'])  # Convert weight decay to float
    ...
    if optimizer_config['USE_SAM']:
        ...
        optimizer = SAM(
            ...
            base_optimizer,
            lr=lr,
            weight_decay=weight_decay,  # Use converted float value
            # Add any additional parameters required by your SAM implementation
        )
...
#     # 确保学习率是浮点数
#     lr = float(optimizer_config['LR'])
#     weight_decay = float(optimizer_config['WEIGHT_DECAY'])  # 确保是浮点数
#     if optimizer_config.get('USE_SAM', False):
#         optimizer = SAM(model_parameters, base_optimizer, rho=2, adaptive=True,
#                         betas=(0.9, 0.999), weight_decay=weight_decay, lr=lr)
#         scheduler = torch.optim.lr_scheduler.StepLR(optimizer.base_optimizer,
#                                                     step_size=scheduler_config['STEP_SIZE'],
#                                                     gamma=scheduler_config['GAMMA'])
#     else:
#         optimizer = base_optimizer(model_parameters, lr=lr, weight_decay=weight_decay)
#         scheduler = torch.optim.lr_scheduler.StepLR(optimizer,
#                                                     step_size=scheduler_config['STEP_SIZE'],
#                                                     gamma=scheduler_config['GAMMA'])
#
